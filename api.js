...
const loadGoldPrice = async () => {
    ...
    try {
        // Sử dụng API miễn phí từ goldpricez.com
        const response = await fetch('https://goldpricez.com/api/rates/currency/vnd');
        const data = await response.json();
        
        // Giá vàng per gram 24k
        const pricePerGram = data.price_gram_24k || 4400000;
        // Chuyển sang per lượng (1 lượng = 37.5 gram)
        const pricePerTael = Math.round(pricePerGram * 37.5);
        
        priceEl.innerHTML = formatNumber(pricePerTael);
        
        const now = new Date();
        updateEl.textContent = now.toLocaleTimeString('vi-VN', { 
            hour: '2-digit', 
            minute: '2-digit' 
        });
    } catch (err) {
        console.log('Gold API error, using fallback:', err);
        // Fallback với giá thực tế gần đúng
        const price = 76800000;
